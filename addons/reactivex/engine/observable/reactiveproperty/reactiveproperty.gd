...
func dispose_with(composite_disposable: CompositeDisposable) -> ReactiveProperty:
	composite_disposable.add_raw(self)
	return self
...
static func Merged2(
	...
	return ReadOnlyReactiveProperty.new(
		p1.merge([p2 as Observable]).map(func(_i): return fn.call(p1.Value, p2.Value)),
		fn.call(p1.Value, p2.Value)
	)
...
static func Merged3(
	...
	return ReadOnlyReactiveProperty.new(
		p1.merge([p2 as Observable, p3 as Observable]).map(func(_i): return fn.call(p1.Value, p2.Value, p3.Value)),
		fn.call(p1.Value, p2.Value, p3.Value)
	)
