...
func add_attr(name, value) -> String:
	return str(name, "=\"", xml_escape(value), "\" ")

func xml_escape(s) -> String:
	if not (s is String):
		return str(s)
	var esc = s.replace("&", "&amp;")
	esc = esc.replace("<", "&lt;")
...
func _export_test_result(test):
	...
	if(test.status == 'pending'):
		...
		var msg = test.failing[0] if test.failing.size() > 0 else "failed"
		var fail_tag = str("<failure ", add_attr("message", msg), "/>")
		to_return += fail_tag
